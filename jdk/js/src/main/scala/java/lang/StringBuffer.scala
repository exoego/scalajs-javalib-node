 ...
 */
class StringBuffer private (builder: JStringBuilder)
    extends AnyRef
    with CharSequence
    with Appendable
    with java.io.Serializable
    with java.lang.Comparable[StringBuffer] {

  def this() = this(new JStringBuilder())
  def this(str: String) = this(new JStringBuilder(str))
  def this(capacity: Int) = this(new JStringBuilder(capacity))
  def this(seq: CharSequence) = this(seq.toString)

  /** A helper so that we can write stuff like `withThisResult(append(obj))`. */
  @inline
  private def withThisResult(op: JStringBuilder): StringBuffer = this
  ...
  override def compareTo(another: StringBuffer): Int = {
    if (this == another)
      0
    else
