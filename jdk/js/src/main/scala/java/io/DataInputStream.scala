...
class DataInputStream(in: InputStream) extends FilterInputStream(in) with DataInput {
  ...
  def readFully(buffer: Array[Byte], offset: Int, length: Int): Unit = {
    if (length < 0) throw new IndexOutOfBoundsException
    if (length == 0) return
    if (in == null) throw new NullPointerException
    if (buffer == null) throw new NullPointerException
    if (offset < 0 || offset > buffer.length - length) throw new IndexOutOfBoundsException

    var remaining = length
    var off       = offset
    while (remaining > 0) {
      val readCount = read(buffer, off, remaining)
      if (readCount == -1) eof()
      remaining -= readCount
      off += readCount
    }
  ...
  @deprecated("Use BufferedReader.readLine", "JDK1.1")
  def readLine(): String =
    throw new UnsupportedOperationException("readLine not implemented since it is deprecated")
      ...
      val char = {
        if ((a & 0x80) == 0x00) { // 0xxxxxxx
          ...
          if ((c & 0xC0) != 0x80) // 10xxxxxx
            badFormat(f"Expected 3 bytes, found: $b%#02x, $c%#02x (init: $a%#02x)")
  ...
  final def skipBytes(n: Int): Int = skip(n.toLong).toInt
...
object DataInputStream {
  def readUTF(in: DataInput): String = {
    decodeUTF(in.readUnsignedShort, in)
  }

  private def decodeUTF(utfSize: Int, in: DataInput) = {
    val buf = new Array[Byte](utfSize)
    val out = new Array[Char](utfSize)
    in.readFully(buf, 0, utfSize)
    Util.convertUTF8WithBuf(buf, out, 0, utfSize)
  }
