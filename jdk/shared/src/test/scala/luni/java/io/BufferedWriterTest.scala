...
class BufferedWriterTest extends AnyFunSuite with BeforeAndAfterEach {
  ...
  ignore("ConstructorLjava_io_Writer") {
  ...
  ignore("ConstructorLjava_io_WriterI") {
  ...
  ignore("writeLjava_lang_StringII") {
    bw.write(testString)
    bw.flush()
    assert(sw.toString == testString, "Incorrect string written")
  }

  ignore("write_LStringII_Exception") {
    val bWriter = new BufferedWriter(sw)
